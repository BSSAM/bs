...
class CurrenciesController extends AppController
{
    ...
    public function index($id=NULL)
    {
        ...
        if($id == '1'):
        $data = $this->Currency->find('all',array('conditions'=>array('Currency.is_deleted'=>1)),array('order' => array('Currency.id' => 'DESC')));
    ...
    public function retrieve($id)
    {
        ...
        if($id!='')
        {
            if($this->Currency->updateAll(array('Currency.is_deleted'=>0),array('Currency.id'=>$id)))
            {
            $this->Session->setFlash(__('The Currency has been Retrieved',h($id)));
            return $this->redirect(array('action'=>'index'));
            }
        }
    }
