...
class AdditionalchargesController extends AppController
{
    ...
    public function index($id=NULL)
    {
        ...
        if($id == '1'):
        $data = $this->Additionalcharge->find('all',array('conditions'=>array('Additionalcharge.is_deleted'=>1)),array('order' => array('Additionalcharge.id' => 'DESC')));
    ...
    public function retrieve($id)
    {
        ...
        if($id!='')
        {
            if($this->Additionalcharge->updateAll(array('Additionalcharge.is_deleted'=>0),array('Additionalcharge.id'=>$id)))
            {
            $this->Session->setFlash(__('The Additional Charge has been Retrieved',h($id)));
            return $this->redirect(array('action'=>'index'));
            }
        }
    }
