...
class TallyledgersController extends AppController
{
    ...
    public function index($id=NULL)
    {
        ...
        if($id == '1'):
        $data = $this->Tallyledger->find('all',array('conditions'=>array('Tallyledger.is_deleted'=>1)),array('order' => array('Tallyledger.id' => 'DESC')));
    ...
    public function retrieve($id)
    {
        ...
        if($id!='')
        {
            if($this->Tallyledger->updateAll(array('Tallyledger.is_deleted'=>0),array('Tallyledger.id'=>$id)))
            {
            $this->Session->setFlash(__('Tally Ledger A/C has been Retrieved',h($id)));
            return $this->redirect(array('action'=>'index'));
            }
        }
    }
