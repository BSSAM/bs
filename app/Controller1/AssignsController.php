...
class AssignsController extends AppController
{
    ...
    public function index($id=NULL)
    {
        ...
        if($id == '1'):
        $data = $this->Assign->find('all',array('conditions'=>array('Assign.is_deleted'=>1)),array('order' => array('Assign.id' => 'DESC')));
    ...
    public function retrieve($id)
    {
        ...
        if($id!='')
        {
            if($this->Assign->updateAll(array('Assign.is_deleted'=>0),array('Assign.id'=>$id)))
            {
            $this->Session->setFlash(__('The Assigned To has been Retrieved',h($id)));
            return $this->redirect(array('action'=>'index'));
            }
        }
    }
